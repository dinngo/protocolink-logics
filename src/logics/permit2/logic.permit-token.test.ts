...
describe('Permit2 PermitTokenLogic', function () {
  ...
  const logic = new PermitTokenLogic(chainId);
  ...
  context('Test build', function () {
    ...
    testCases.forEach(({ erc20Funds }, i) => {
      it(`case ${i + 1}`, async function () {
        const permitData = await logic.getPermitData(account, erc20Funds);
        const routerLogic = await logic.build({ permit: permitData!.values, sig: permitSig }, { account });

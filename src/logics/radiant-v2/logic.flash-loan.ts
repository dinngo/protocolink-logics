...
import { LendingPool__factory, RadiantV2FlashLoanCallback__factory } from './contracts';
...
export class FlashLoanLogic extends core.Logic implements core.LogicTokenListInterface, core.LogicBuilderInterface {
  ...
  get callbackAddress() {
    return getContractAddress(this.chainId, 'RadiantV2FlashLoanCallback');
  }

  async calcCallbackFee(loan: common.TokenAmount) {
    const callback = RadiantV2FlashLoanCallback__factory.connect(this.callbackAddress, this.provider);
